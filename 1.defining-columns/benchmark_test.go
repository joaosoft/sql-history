...
func Benchmark(b *testing.B) {
	...
	historyTableName := "example_one"
	modelSchemaName := "model"
	modelTableName := "example_one"
	...
	for i := 1; i <= expectedAffectedRows; i++ {
		result, err := tx.Exec(
			fmt.Sprintf(`INSERT INTO %s.%s (id_example_one, name, description) 
	...
	row := db.QueryRow(fmt.Sprintf("SELECT COUNT(id_example_one) FROM %s.%s", historySchemaName, historyTableName))
