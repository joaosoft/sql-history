...
func Benchmark(b *testing.B) {
	...
	historyTableName := "example_three"
	modelSchemaName := "model"
	modelTableName := "example_three"
	...
	for i := 1; i <= expectedAffectedRows; i++ {
		result, err := tx.Exec(
			fmt.Sprintf(`INSERT INTO %s.%s (id_example_three, name, description) 
	...
	row := db.QueryRow(fmt.Sprintf("SELECT COUNT(id_example_three) FROM %s.%s", historySchemaName, historyTableName))
