...
func Benchmark(b *testing.B) {
	...
	historyTableName := "example_four"
	modelSchemaName := "model"
	modelTableName := "example_four"
	...
	for i := 1; i <= expectedAffectedRows; i++ {
		result, err := tx.Exec(
			fmt.Sprintf(`INSERT INTO %s.%s (id_example_four, name, description) 
	...
	row := db.QueryRow(fmt.Sprintf("SELECT COUNT(id_example_four) FROM %s.%s", historySchemaName, historyTableName))
