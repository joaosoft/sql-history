...
func Benchmark(b *testing.B) {
	...
	historyTableName := "example_two"
	modelSchemaName := "model"
	modelTableName := "example_two"
	...
	for i := 1; i <= expectedAffectedRows; i++ {
		result, err := tx.Exec(
			fmt.Sprintf(`INSERT INTO %s.%s (id_example_two, name, description) 
	...
	row := db.QueryRow(fmt.Sprintf("SELECT COUNT(id_example_two) FROM %s.%s", historySchemaName, historyTableName))
