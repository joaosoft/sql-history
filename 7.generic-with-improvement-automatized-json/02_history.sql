...
LOOP
	IF substring(_rec.column_name, 1, 1) <> '_' THEN
        ...
        _values := _values || format('%L', _row->>_rec.column_name);
	END IF;
END LOOP;
...
BEGIN
	...
	IF (TG_OP = 'INSERT') THEN
        ...
        EXECUTE(history.create_insert_history_statement(_history_schema_name, _history_table_name, _model_schema_name, _model_table_name, TO_JSON(NEW), 'I'));
        RETURN NEW;
    ELSIF (TG_OP = 'UPDATE') THEN
      -- catch errors
        ...
        EXECUTE(history.create_insert_history_statement(_history_schema_name, _history_table_name, _model_schema_name, _model_table_name, TO_JSON(NEW), 'U'));
        RETURN NEW;
    ELSIF (TG_OP = 'DELETE') THEN
        ...
        EXECUTE(history.create_insert_history_statement(_history_schema_name, _history_table_name, _model_schema_name, _model_table_name, TO_JSON(OLD), 'D'));
        RETURN OLD;
    END IF;
	EXCEPTION
	WHEN OTHERS THEN
...
SELECT history.create_history_table('history', 'example_seven', 'model', 'example_seven');